...
def rag_tool(query:str) -> str:
    """
    Retrieve relevant inforation from the pdf documnet.

    Use this tool when the user asks factual or conceptual questions 
    that may be answered useng the stored PDF document.
    ...
    if not documents:
        return "No relevant information found in the PDF document."
...
def get_stock_price(symbol: str) -> dict:
    ...
    """
    url = f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey=FSV20HHBJITXFLH2"
    r = requests.get(url)
    return r.json()
...
def purchase_stock(symbol: str, quantity: int) -> dict:
    ...
    HUMAN - IN - THE - LOOP:
    Before configuring the puurchase, this toll will interrupt 
    and wait for a human decision ("yes" or "no") to proceed with the stock purchase.

    """

    # This pause the graph and return control to the caller 
    decision = interrupt(f"Approve buying {quantity} shares of {symbol}? (yes/no)")

    if isinstance(decision, str) and decision.lower() == "yes":
        return {
            "status": "success",
            "message": f"Purchased order placed for {quantity} shares of {symbol}.",
            "symbol":symbol,
            "quantity":quantity
        }
        ...
        return {
            "status": "cancelled",
            "message": f"Purchase order for {quantity} shares of {symbol} was cancelled.",
            "symbol":symbol,
            "quantity":quantity
        }
